...
namespace ADO.NET_Example
{
    public partial class DBUtilityDemo : System.Web.UI.Page
    {
        ...
        protected async void btnColor_Click(object sender, EventArgs e)
        {
            ...
            if (Page.IsValid)
            {
                cmdParameters["nm"] = new SqlParameter("nm", txtColor.Text);
                cmdParameters["hex"] = new SqlParameter("hex", txtHex.Text);
                ltReport.Text = await transactionEx.InsertDBAsync("TestDBConnectionString", cmdParameters, cancellationToken.Token); //asyncADO.InsertColorUsingProviderModel("TestDBConnectionString", cmdParameters); 
